...
Two-stage pipeline:
    ...
    Stage 1 — Enhancement   (right panel, display only)
        SS-UIE (CVPR 2023) or WaterNet neural base
        → LAB correction + CLAHE + MSR blend
...
from enhance import enhance, load_ssuie, load_waternet

IMAGE_FOLDER   = "images"
OUTPUT_FOLDER  = "outputs"
MODEL_PATH     = "fish_model.pt"
SSUIE_WEIGHTS  = "SS_UIE.pth"
...
def process(path, yolo_model, ssuie_bundle, waternet_bundle):
    img=cv2.imread(path)
    if img is None: print(f"[error] cannot read {path}"); return
    filename=os.path.basename(path)

    enh  = enhance(img, ssuie_bundle=ssuie_bundle, waternet_bundle=waternet_bundle)
...
def main():
    ...
    # Load neural enhancement models (graceful fallback if unavailable)
    ssuie_bundle    = load_ssuie(SSUIE_WEIGHTS)    if os.path.exists(SSUIE_WEIGHTS)    else None
    waternet_bundle = load_waternet()

    mode = ("SS-UIE + WaterNet + classical"  if ssuie_bundle and waternet_bundle else
            "SS-UIE + classical"             if ssuie_bundle else
            "WaterNet + classical"           if waternet_bundle else
            "classical (MSR + FFT + guided)")
    ...
    for fname in images:
        process(os.path.join(IMAGE_FOLDER,fname),
                yolo_model, ssuie_bundle, waternet_bundle)

    print(f"\nfinished — {len(images)} image(s) → '{OUTPUT_FOLDER}/'")
